...
export default class Uploader {
  ...
  */
  constructor({ config, onUpload, onError }) {
    ...
    if (config.uploader || config.endpoint)
      console.warn("config.uploader & endpoint options are ignored");
  }
  
  async customHandler(file) {
    this.file = file;
    let resp = {
      success: 1,
      file: {
  ...
  uploadSelectedFile({ onPreview }) {
    ...
    */
    ...
    upload = ajax.selectFiles({ accept: this.config.types }).then((files) => {
      onPreview();
      const customUpload = this.customHandler(files[0]);
